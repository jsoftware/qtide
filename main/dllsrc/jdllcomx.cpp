...
static void showapp(long b);
static void shutdownOLE();
extern BOOL quitflag;
static long runit(WCHAR* p, BOOL f);
static void oleload();
...
static void touni(C* src, WCHAR* sink)
{
  while(*src)
    *sink++ = *src++;
  *sink++=0;
...
static void toasc(WCHAR* src, C* sink)
{
  while(*src)
    *sink++ = (char)*src++;
  *sink++=0;
...
static int toutf8n(WCHAR* src, LPSTR snk, int snklen)
{
  LPSTR p;
...
static void oleload()
{
  ...
  JInt64R=(JInt64RType)GETPROCADDRESS((HMODULE)hjdll,"JInt64R");
}
#endif // _JDLL
...
void ObjectDestroyed(void)
{
  g_cObj--;
#ifndef _JDLL
  if(!g_cObj && !g_cLock && quitflag)	shutdownOLE();
#endif
  return;
}

...
typedef int (*Run)(int,char **,char *,bool,int,void *,void *);
...
{
  if (state_run && argc<0)
    return state_run(argc,0,arg1,false,arg2,0,0);
  else
    return 0;
}
...
int main(int argc, char *argv[])
{
  ...
  if (state_run) {
    state_run(argc, argv, lib->fileName().toUtf8().data(),fhs,(embedding)?0:1,0,(void *)-1);
#if defined(_WIN32)
    if (embedding)
      if (!initexeserver())
        return -1;
...
#else
    return state_run(-1,0,0,false,0,0,0);
#endif
  }
